...
impl AppState {
    ...
    fn enable_description(&mut self) {
        if let Some(list_node) = self.get_selected_node() {
            let mut preview_title = "Command Description - ".to_string();
            preview_title.push_str(list_node.name.as_str());
            let description = FloatingText::new(&list_node.description, &preview_title);
    ...
    fn toggle_task_list_guide(&mut self) {
        self.spawn_float(
            FloatingText::new(ACTIONS_GUIDE, "Important Actions Guide"),
            80,
            80,
        );
    }
